...
import { CreateVideoAvatarForm } from "@/components/dashboard/video-avatars/create-video-avatar-form";
...
export default async function NewAiVideoAvatarPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <Button nativeButton={false} render={<Link href="/dashboard/ai-video-avatar" />} size="sm" variant="ghost">
          <ArrowLeftIcon />
          Back to AI Video Avatars
        </Button>
      </div>

      <header>
        <p className="text-sm font-semibold tracking-[0.18em] text-primary/75">AI VIDEO AVATAR</p>
        <h1 className="mt-2 font-heading text-3xl font-semibold tracking-[-0.04em] text-foreground">Create AI Video Avatar</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          Combine a script, an avatar, and a voice into a talking-head video.
        </p>
      </header>

      <CreateVideoAvatarForm avatars={avatars} creditsBalance={creditsBalance} customVoices={customVoices} defaultVoices={defaultVoices} />
    </div>
  );
}
